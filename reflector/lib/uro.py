...
def dict_to_params(params: dict, temperValue: str):
    """
    ...
    url["original_url"] = '?' + '&'.join(stringed)

    stringed = [name + '=' +
                str(i+1) + temperValue for name, i in zip(params, range(len(params)))]
    url["tempered_url"] = '?' + '&'.join(stringed)
    return url
...
def search_unique_urls(file_path):
    with open(file_path, "r") as file:
        for line in file:
            ...
            has_new_param = False if not params else is_new_param(
                params.keys())
            new_params = [param for param in params.keys()
...
def get_unique_urls(file_path, with_only_query=True, temperValue="qwerasdf"):
    urls = []
    search_unique_urls(file_path=file_path)
    for host, value in urlmap.items():
        for path, params in value.items():
            if params:
                for param in params:
                    u = {}
                    for k, v in dict_to_params(param, temperValue).items():
                        u[k] = host + path + v
                    urls.append(u)
            else:
                if not with_only_query:
                    urls.append({"original_url": host + path})
    return urls


# print(get_unique_urls("lib/urls.txt"))
